...
# pg_table = pd.read_sql_query('''SELECT * FROM pinterest_data''', engine)

# pg_table_sch = pg_table.printSchema
...
# my_schema = ArrayType(StructType().add("category", StringType(), True).add("unique_id", StringType(), True) \
#     .add("title", StringType(), True) \
#     .add("description", StringType(), True) \
#     .add("follower_count", StringType(), True) \
#     .add("tag_list", StringType(), True) \
#     .add("is_image_or_video", StringType(), True) \
#     .add("image_src", StringType(), True) \
#     .add("downloaded", IntegerType(), True) \
#     .add("save_location", StringType(), True))

# my_schema2 = ArrayType(StringType())   


os.environ["PYSPARK_SUBMIT_ARGS"] = '--packages org.apache.spark:spark-sql-kafka-0-10_2.12:3.2.1 example_spark_streaming_v1.py pyspark-shell'
